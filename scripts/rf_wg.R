...
output_dir <- "/home/minhao/knn/xinxinxinknn/moxing/rf/wgmlst3586"
shared_fold_file <- "/home/minhao/knn/xinxinxinknn/daima/rf_shared_5fold_assignment.tsv"
model_file <- "/home/minhao/knn/xinxinxinknn/moxing/rf/wgmlst3586/rf_wgmlst3586_model.rds"
...
log_message("===== Random Forest wgmlst3586 5-fold CV started =====")
...
if (nfeat != 3586L) {
  stop(sprintf("Expected 3586 features, but found %d", nfeat))
}
...
fwrite(label_counts, file.path(output_dir, "label_counts_wgmlst3586.tsv"), sep = "	")
...
saveRDS(folds, file.path(output_dir, "folds_5cv_wgmlst3586.rds"))
fwrite(fold_assignment, file.path(output_dir, "fold_assignment_wgmlst3586.tsv"), sep = "	")
...
fwrite(metrics_by_fold, file.path(output_dir, "metrics_by_fold_wgmlst3586.tsv"), sep = "	")
fwrite(classwise_by_fold, file.path(output_dir, "classwise_by_fold_wgmlst3586.tsv"), sep = "	")
fwrite(oof_probabilities, file.path(output_dir, "oof_probabilities_wgmlst3586.tsv"), sep = "	")
...
fwrite(metrics_summary, file.path(output_dir, "metrics_summary_wgmlst3586.tsv"), sep = "	")
...
fwrite(classwise_summary, file.path(output_dir, "classwise_summary_wgmlst3586.tsv"), sep = "	")
...
fwrite(parameter_table, file.path(output_dir, "fixed_parameters_wgmlst3586.tsv"), sep = "	")
...
metric_plot <- ggplot(metric_long, aes(x = Metric, y = Value)) +
  ...
  labs(title = "Random Forest wgmlst3586 5-fold CV", x = NULL, y = "Score") +
  theme_minimal(base_size = 15) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

ggsave(file.path(output_dir, "rf_wgmlst3586_5fold_metrics.png"), metric_plot, width = 8, height = 5, dpi = 300)

recall_plot <- ggplot(classwise_summary, aes(x = Class, y = Mean_Recall)) +
  geom_col(width = 0.7) +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Random Forest wgmlst3586 class-wise recall", x = "Class", y = "Mean Recall") +
  theme_minimal(base_size = 15) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5, face = "bold"))

ggsave(file.path(output_dir, "rf_wgmlst3586_recall_by_class.png"), recall_plot, width = 10, height = 6, dpi = 300)
...
summary_file <- file.path(output_dir, "run_summary_wgmlst3586.txt")
writeLines(c(
  "===== Random Forest 5-fold CV summary =====",
  sprintf("Created at: %s", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  sprintf("Dataset: %s", "wgmlst3586"),
  ...
  "",
  "Output files:",
  sprintf("metrics_by_fold_%s.tsv", "wgmlst3586"),
  sprintf("metrics_summary_%s.tsv", "wgmlst3586"),
  sprintf("classwise_by_fold_%s.tsv", "wgmlst3586"),
  sprintf("classwise_summary_%s.tsv", "wgmlst3586"),
  sprintf("oof_probabilities_%s.tsv", "wgmlst3586"),
  sprintf("fold_assignment_%s.tsv", "wgmlst3586"),
  sprintf("fixed_parameters_%s.tsv", "wgmlst3586"),
  sprintf("rf_%s_5fold_metrics.png", "wgmlst3586"),
  sprintf("rf_%s_recall_by_class.png", "wgmlst3586"),
  sprintf("Model file: %s", model_file)
), summary_file)

log_message("===== Random Forest wgmlst3586 DONE =====")
log_message("Metrics summary: %s", file.path(output_dir, "metrics_summary_wgmlst3586.tsv"))
